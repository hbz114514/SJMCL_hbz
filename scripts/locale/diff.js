...
function checkTodoValues(locale, localeName) {
  ...
  if (todoValues.length > 0) {
    ...
    todoValues.forEach((key) => {
      logger(chalk.yellow(`  %TODO found: ${key}`));
    });
...
function compareLocales(base, target) {
  ...
  if (!fs.existsSync(targetLocalePath)) {
    logger(`Locale file '${target}.json' not found in: ${localesPath}`);
    return;
  }
  ...
  if (missing.length === 0 && extra.length === 0) {
    logger(chalk.green(`✅ '${target}.json' is identical to '${base}.json'.`));
  } else {
    logger(`Comparing ${target} to ${base}:`);
    logger(`${missing.length} missing, ${extra.length} extra keys`);

    missing.forEach((key) => {
      logger(chalk.red(`  Missing:   ${key}`));
    });
    extra.forEach((key) => {
      logger(chalk.green(`  Extra:     ${key}`));
    });
...
function compareAll(baseLocaleName) {
  ...
  if (!fs.existsSync(baseLocalePath)) {
    logger(`Locale file '${baseLocaleName}.json' not found in: ${localesPath}`);
    return;
  }
